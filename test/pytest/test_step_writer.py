...
def test_number_of_files_per_folder(tmp_path):
        ...
        for dirpath in root.rglob("*"):
            if dirpath.is_dir():
                ...
                file_count = sum(
                    1
                    for item in items
                    if item.is_file() and item.name.endswith(".ddh5")
                )
    ...
    with DDH5Writer(
        datadict, str(tmp_path), name="first", safe_write_mode=True
    ) as writer:
        for i in range(n_files):
            writer.add_data(x=i, y=i**2, z=i**3)
    ...
    with DDH5Writer(
        datadict, str(tmp_path), name="second", safe_write_mode=True
    ) as writer:
        for i in range(n_files):
            writer.add_data(x=i, y=i**2, z=i**3)
...
def test_basic_unification(tmp_path, n_files=500):
    ...
    datadict_correct = DataDict(
        x=dict(unit="m"), y=dict(unit="m"), z=dict(axes=["x", "y"])
    )
