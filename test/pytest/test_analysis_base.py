...
def test_init_savefolder_structure(tmp_path):
    ...
    analysis = DatasetAnalysis(
        datafolder, name="myanalysis", analysisfolder=tmp_path / "analysis"
    )

    assert (
        analysis.savefolders[0]
        == tmp_path / "analysis" / "myanalysis" / datafolder.stem
    )
...
def test_analysis_name_groups_multiple_datasets(tmp_path):
    ...
    for folder_name, analysis_name in runs:
        ...
        datafolder.mkdir()
        with DatasetAnalysis(
            datafolder, name=analysis_name, analysisfolder=analysisfolder
        ) as a:
