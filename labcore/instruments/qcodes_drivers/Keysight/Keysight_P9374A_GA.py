...
@author: Hatlab: Ryan Kaufman, Gaurav Agarwal; UIUC: Wolfgang Pfaff
...
class Keysight_P9374A_SingleChannel(VisaInstrument):
    ...
    def __init__(self, name, address=None, **kwargs):
        ...
        logging.info(__name__ + ' : Initializing instrument $@$&% PNA')

        super().__init__(name, address, terminator='\n', **kwargs)

        self.write('CALC1:PAR:MNUM 1')  # sets the active msmt to the first channel/trace
        self.del_time = 0.2  # From Hat_P9374A_RK
        self.last_msmt_msg = None
        # Add in parameters
    ...
    # def average(self, number: int = None, avg_over_freq: bool = False) -> Tuple[np.ndarray, str, str, int]:
        """
        Do the average number (if provided) else, self.avg_num() times.
        ...
        self.averaging(1)  #turn on averaging
        self.write("SENS:AVER:CLE")  # does not apply to point averaging
        total_time = self.sweep_time() * self.avg_num() + 0.5
        print(f"Waiting {np.round(total_time, 1)}s for {self.avg_num()} averages...")
        ...
        # if avg_over_freq:
        #     data = np.average(self.trace_1.data(), axis=1).reshape((2, 1))
        # else:
        #     data = self.trace_1.data()
        self.trigger_source(prev_trigger_source)
        self.sweep_mode(prev_sweep_mode)
        self.averaging(prev_averaging)
    ...
    def renormalize(self, num_avgs: int, pwr_bump: float = 0):
        self.power(self.power() + pwr_bump)
        self.averaging(1)
        self.avg_num(num_avgs)
