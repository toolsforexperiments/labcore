...
For a Sweep to know that either a **pointer** (an iterable object), or an **action** (a callable object) they need to be wrapped by an instance of {py:class}`DataSpec <labcore.measurement.record.DataSpec>`.
{py:class}`DataSpec <labcore.measurement.record.DataSpec>` is a [data class](https://docs.python.org/3/library/dataclasses.html) that holds information about the variable itself.
...
The important fields of a {py:class}`DataSpec <labcore.measurement.record.DataSpec>` are its `name` and `depends_on` fields.
`name` simply indicates the name of the variable, e.i. the key that the sweep will have for the value of this variable.
`depends_on` indicates whether the variable is an independent variable (we control) or a dependent variable (the things we are trying to measure).
...
:::{note}
{py:class}`DataSpec <labcore.measurement.record.DataSpec>` also contains two more fields: `unit` and `types`, these however, have no impact in the way the code behaves and are for adding extra metadata for the user.
:::
...
To wrap functions we use the {py:func}`recording <labcore.measurement.record.recording>` decorator on the function we want to annotate:
...
A nicer way of creating {py:class}`DataSpec <labcore.measurement.record.DataSpec>` instances is to use the functions {py:func}`independent <labcore.measurement.record.independent>` and {py:func}`dependent <labcore.measurement.record.dependent>`.
This function just makes the recording of data easier to read.
{py:func}`independent <labcore.measurement.record.independent>` does not let you indicate the ``depends_on`` field while {py:func}`dependent <labcore.measurement.record.dependent>`, has an empty list (indicating that it depends an all other independents) as a default.
...
You can also use the shorter versions of these functions:
    * {py:func}`labcore.measurement.record.ds` for shorter {py:class}`DataSpec <labcore.measurement.record.DataSpec>`
    * {py:func}`labcore.measurement.record.indep` for shorter {py:func}`independent <labcore.measurement.record.independent>`
    * {py:func}`labcore.measurement.record.dep` for shorter {py:func}`dependent <labcore.measurement.record.dependent>`
:::
...
For this we can use the function {py:func}`record_as <labcore.measurement.record.record_as>` to annotate any function or generator on the fly:
...
```
You can add multiple {py:class}`DataSpec <labcore.measurement.record.DataSpec>` with in a single {py:func}`record_as <labcore.measurement.record.record_as>`:
...
Once the Sweep is created we can see the **records** it will produce by using the function method {py:meth}`get_data_specs <labcore.measurement.sweep.Sweep.get_data_specs>`:
...
If you are trying to sweep over a single parameter, a more convenient syntax for creating Sweep is to utilize the {py:func}`sweep_parameter <labcore.measurement.sweep.sweep_parameter>` function:
...
If we don't want to resort to global variables we can do so by using the method {py:meth}`set_options <labcore.measurement.sweep.Sweep.set_options>`.
...
:::{note}
{py:attr}`record_none <labcore.measurement.sweep.Sweep.record_none>` controls whether we include data fields that have returned nothing during setting a pointer or executing an action.
...
* Appending: {py:func}`append_sweeps <labcore.measurement.sweep.append_sweeps>`
* Multiplying: {py:func}`zip_sweeps <labcore.measurement.sweep.zip_sweeps>`
* Nesting: {py:func}`nest_sweeps <labcore.measurement.sweep.nest_sweeps>`
:::
...
:::{note}
There are two different Sweep configuration related to passing arguments in Sweeps. For more information on them see the [Configuring Sweeps](#configuring-sweeps).
:::
...
A simple way of renaming conflicting arguments and **records** is to use the combination of ``lambda`` and {py:func}`record_as <labcore.measurement.record.record_as>`:
...
The class {py:class}`Sweep <labcore.measurement.sweep.Sweep>` has three global parameters that are used to configure the behavior of it.

### record_none

{py:attr}`record_none <labcore.measurement.sweep.Sweep.record_none>`, ``True`` by default, adds ``None`` to any **action** or **pointer** that didn't generate any **record** that iteration.
...
{py:attr}`pass_on_returns <labcore.measurement.sweep.Sweep.pass_on_returns>`, ``True`` by default, specifies if we want arguments to be passed between sweeps.
...
{py:attr}`pass_on_none <labcore.measurement.sweep.Sweep.pass_on_none>`, ``False`` by default, specifies if variables that return ``None`` should be passed as arguments to other **actions** or Sweeps (Because ``None`` is typically indicating that function did not return anything as data even though a record was declared using {py:func}`recording <labcore.measurement.record.recording>` or {py:func}`record_as <labcore.measurement.record.record_as>`):
...
This is simple but does not do much else. If we only want to store the data generated by a Sweep in disk for later analysis we can use the function {py:func}`run_and_save_sweep <labcore.measurement.storage.run_and_save_sweep>`:
...
{py:func}`run_and_save_sweep <labcore.measurement.storage.run_and_save_sweep>` automatically runs the Sweep indicated, stores the **records** generated by it in a :class:`DataDict <plottr.data.datadict.DataDict>` in a ddh5 file with time tag followed by a random sequence followed by the third argument, in the directory passed by the second argument.
...
:::{note}
{py:func}`run_and_save_sweep <labcore.measurement.storage.run_and_save_sweep>` can save multiple objects to disk by accepting them as extra arguments.
...
If we also need this **action** to be a Sweep, the function {py:func}`once <labcore.measurement.sweep.once>` will create a Sweep with no **pointer** that runs an **action** a single time:
